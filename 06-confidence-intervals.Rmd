...
a banana weighs `r round(mean_of_means, 0)` on average, 
...
Unlike Cy, who sampled repeatedly from the population of all bananas, 
...
does not have obvious advantages over sampling from the population. 
...
The mean of these 1000 means is `r mean_of_means` grams, 
...
`mean_of_means` = `r mean_of_means` grams. 
...
`r bootstrap_se` is an approximation 
...
I created `r n_pop` plausible numbers representing the weight of `r n_pop` bananas. 
The numbers are stored in the data frame `banana_pop`. 

```{r banana-pop, eval=F}
dget("https://raw.githubusercontent.com/chunyunma/baby-modern-dive/master/data/banana_pop.txt")
