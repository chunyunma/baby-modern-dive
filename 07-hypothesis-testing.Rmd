...
to understand hypothesis testing in Chapters \@ref(clt) 
and \@ref(confidence-intervals). 
...
md <- import::from(moderndive, .all=TRUE, .into={new.env()})
...
<!-- Change the hypothesis at the beginning to directional. TODO -->
...
infer::visualize(null_distribution, bins = 10) + 
  ...
  gg$labs(x = expression(paste("Difference in sample proportions ", hat(p)["m"] - hat(p)["f"])), 
          title = "significance level (alpha value, light + bright pink region) vs. \n p value (bright pink region) ")
```
...
Now, ask yourself, 
how does the observed $z$-score, `r z_score`, fare compared to
...
```{r comparing-diff-prop-z-2, echo=F, message=T, fig.align='center', fig.cap="Comparing the null distributions of two test statistics."}
# simulated-based null distribution
null_dist_1 <- infer::visualize(null_distribution, bins = 10) + 
  infer::shade_p_value(obs_stat = observed_test_statistic, 
                       direction = "right", 
                       fill = c("hotpink"), 
...
<!-- [TODO]: apprently this is only good for sim-based -->
<!-- we can apply `infer::get_p_value()` to the null distribution:  -->
...
is of the same magnitude as the one we obtained earlier 
...
### Conducting a $z$-test using base-R functions
...
tibble::tibble(
 ...
 # tab_header(title = "Type I and Type II errors hypothesis tests") %>%
  ...
  gt::tab_spanner(
    label = "Truth",
    columns = gt::vars(`H0 true`, `HA true`)
  ) %>% 
  gt::cols_align(align = "center") %>% 
  gt::tab_options(table.width = gt::pct(90))
```
...
and their corresponding confidence intervals in Appendix \@ref(??).
