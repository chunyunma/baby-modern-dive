...
In Chapter \@ref(regression) on basic regression, 
...
<!-- Let's now load all the packages needed for this chapter ... -->
...
df_gapminder2007 <- gapminder::gapminder %>%
  ...
  # [TODO] need to explain the next line
  dp$mutate(income = log10(gdp_per_cap)*10000)
```
...
can be found in its associated help file (run `?gapminder` in the console). 
...
some of the dots deviated, such as the ones highlighted in red in figure xx. 
...
\vspace{-0.1in}
```
Compare Figure \@ref(fig:numxplot1) to the following:
