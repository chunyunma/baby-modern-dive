...
Confirm that `renv` is working properly by typing `.libPahts()` again 
...
in the R community. 
I personally summarize it as "to load or not to load". 
...
To access any function within the package, 
you can type `ggplot2::geom_point`, as in `PACKAGE::FUNCTION`. 
...
you may be thinking: what about using `library()` to load a package? 
This is why I said "controversial" at the beginning. 
Please allow me to elaborate. 

There are two types of `R` users. 
Those that load packages using `library()`, 
and those that do not load packages. 
Based on my estimate, 99% of `R` users fall into first category. 
Until three months ago, I was also a "loader." 
...
I did not take it very seriously and just reassured the student that 
...
how can I expect learners new to `R` not frustrated by it? 
...
* There are some `R` functions that come with every clearn install of `R`, 
...
* Other of the lecture notes, you will be assigned other reading material 
  and exercises. 
...
As of 2021 January, the latest R version is `4.0.3`. 
