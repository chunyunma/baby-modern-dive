...
We'll investigate if, on average, 
...
$`r x_bar_action %>% round(3)` - `r x_bar_romance %>% round(3)` = 
`r (x_bar_action - x_bar_romance) %>% round(3)`$. 

So there appears to be a slight difference of 
`r (x_bar_action - x_bar_romance) %>% round(3)` in favor of romance movies. 
...
that is located in the **critical region**, 
we conclude that the data are not consistent with the null hypothesis, 
and we reject the null hypothesis.
<!-- citation -->
<!-- [^2]: Gravetter, F. J., & Wallnau, L. B. (2011). Essentials of statistics for the behavioral sciences. In Essentials of statistics for the behavioral sciences (7th ed, pp. 206–207). Wadsworth/Cengage Learning. -->
...
So how can we use the two-sample $t$-statistic as a test statistic 
...
how many data points are involved in coomputing a sample statistic, 
...
subtitute each variable in Equation \@ref(t-mean-diff-2) with 
...
one point of actual difference 
...
A key value included in this 99.9% confidence interval for 
...
#### Randomization for hypothesis test using `infer` verbs {-}
...
that are greater than or equal to `r income_diff` or less than or equal 
to `r -income_diff` for our $p$-value.
...
boot_distn_two_means %>%
  infer::visualize(endpoints = ci, direction = "between")
```
...
we do not have evidence that Sacramento incomes are different than Cleveland incomes.

**Interpretation**: 
We are 95% confident the true mean yearly income 
for those living in Sacramento is between 
`r round(-ci[["lower_ci"]], 2)` dollars 
smaller to `r round(ci[["upper_ci"]], 2)` dollars higher than for Cleveland.
...
We have some reason to doubt the normality assumption here since both the histograms show deviation from a normal model fitting the data well for each group. 
The sample sizes for each group are greater than 100 though so the assumptions should still apply.
...
We do not have evidence to suggest that the true mean income differs 
between Cleveland, OH and Sacramento, CA based on this data.
