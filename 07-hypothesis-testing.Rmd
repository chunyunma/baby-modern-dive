...
tibble::tibble(
  Decision = c("$$\\text{Fail to reject } H_0$$", "$$\\text{Reject } H_0$$"), 
  `$$H_0 \\text{ true}$$` = c("Correct", "Type I error"),
  `$$H_A \\text{ true}$$` = c("Type II error", "Correct")
) %>% 
 ...
 # tab_header(title = "Type I and Type II errors hypothesis tests") %>%
  ...
  gt::tab_spanner(
    label = "Truth",
    columns = gt::vars(`$$H_0 \\text{ true}$$`, `$$H_A \\text{ true}$$`)
  ) %>% 
  gt::cols_align(align = "center") %>% 
  gt::tab_options(table.width = gt::pct(90))
```
