...
if(!file.exists(here::here("rds", "banana_samples.rds"))){
  ...
  saveRDS(object = banana_samples, here::here("rds", "banana_samples.rds"))
} else {
...
## Constructing a confidence interval {#bootstrap-process}
...
I created `r n_pop %>% formatC(big.mark=",", format="d")` plausible numbers 
representing the weight of `r n_pop %>% formatC(big.mark=",", format="d")` bananas. 
...
```{r hundred-ci, echo=F}
if(!file.exists(here::here("rds", "banana_percentile_95cis.rds"))){
  set.seed(4)
  ...
  saveRDS(object = banana_percentile_cis, here::here("rds", "banana_percentile_95cis.rds"))
} else {
  banana_percentile_cis <- readRDS(here::here("rds", "banana_percentile_95cis.rds"))
}
```
...
```{r echo=F}
if(!file.exists(here::here("rds", "banana_perc_cis_n_25_50_100.rds"))){
  set.seed(7)
  ...
  # Save output to rds object
  readr::write_rds(percentile_cis_by_n, here::here("rds", "banana_perc_cis_n_25_50_100.rds"))
} else {
  percentile_cis_by_n <- readr::read_rds(here::here("rds", "banana_perc_cis_n_25_50_100.rds"))
}
```
...
<!-- This is the same formula notation you saw in Chapters \@ref(simple-regression)  -->
...
if (!file.exists(here::here("rds", "banana_sample_generate.rds"))) {
  banana_sample_generate <- banana_sample %>%
    infer::specify(response = weight) %>%
    infer::generate(reps = 1000, type = "bootstrap")
  readr::write_rds(banana_sample_generate, here::here("rds", "banana_sample_generate.rds"))
} else {
  banana_sample_generate <- readr::read_rds(here::here("rds", "banana_sample_generate.rds"))
}
banana_sample_generate
```

Observe that the resulting data frame has `r (n_sample * n_resample) %>% 
formatC(big.mark = ",", format="d")` rows. 
...
formatC(big.mark=",", format="d")` = `r n_sample` $\cdot$ `r n_resample`. 
...
if (!file.exists(here::here("rds", "bootstrap_distribution_banana.rds"))) {
  bootstrap_distribution <- banana_sample %>%
    ...
    infer::generate(reps = 1000) %>%
    infer::calculate(stat = "mean")
  readr::write_rds(bootstrap_distribution, here::here("rds", "bootstrap_distribution_banana.rds"))
} else {
  bootstrap_distribution <- readr::read_rds(here::here("rds", "bootstrap_distribution_banana.rds"))
}
bootstrap_distribution
```
...
infer::visualize(bootstrap_distribution, boundary=190, binwidth=1, colour="white")
```
