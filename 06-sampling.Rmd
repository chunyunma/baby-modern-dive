...
dput(banana_sample, "data/banana_sample_ch5.txt")
```
...
dput(df_weight_avrg, "data/banana_sampling_ch5.txt")
df_weight_avrg %>% 
  dp$rename(`Average Weight` = weight_avrg) %>% 
  dp$slice_head(n = 20) %>% 
  knitr::kable(
...
of `r round(sd_thousandsample*2, 2)` grams.]{color="rosybrown"}
...
$\pm$ `r round(sd_thousandsample*2, 2)` grams, 
or within [`r round(m_thousandsample - sd_thousandsample*2, 2)`, 
`r round(m_thousandsample + sd_thousandsample*2, 2)`] grams*.]{color="rosybrown"}
...
banana_sample <- dget("https://raw.githubusercontent.com/chunyunma/baby-modern-dive/master/data/banana_sample_ch5.txt")
...
banana_sample %>% 
  dplyr::summarize(sd_onesample = sd(weight))
...
se_empirical <- dplyr::summarize(banana_sample_means, sd(weight_avrg)) %>% 
  round(2)
se_empirical
```
