...
and [*compute*]{color="red"} the sample mean .... 
...
[Based on instinct, she starts by taking one sample of 50 bananas, 
...
# weight <- floor(rnorm(10000, mean = 175, sd = 30))
...
banana_sample <- banana_pop %>%
  #   infer::rep_sample_n(size = 50)
  dplyr::slice_sample(n = 50) %>% 
  dplyr::select(weight)
# Note: can also use sample_n(size = 50, replace = TRUE) to draw a sample. 
# dplyr::slice_sample() is appropriate for dataframes; 
# sample(){base} only works for one-dimensional data, i.e., vectors.
# banana_sample
```
...
hist_onesample <- gg$ggplot(banana_sample, gg$aes(x = weight)) + 
 ...
 gg$geom_density(alpha = 0.2, fill = "#FF6666") + 
  gg$labs(x = "Weight of 50 bananas (gram)", y = "") + 
...
m_onesample <- banana_sample %>% 
  dp$summarize(avrg_weight = mean(weight)) %>% 
  dp$pull(avrg_weight)
```
...
randomly choose a sample of 50 bananas, 
weigh them, calculate the average weight, choose another sample, 
so on and so forth. 
...
[After days' hardwork, 
Cy completed measuring and plotting 1,000 banana samples. ]{color="rosybrown"}

```{r echo = FALSE}
banana_samples <- banana_pop %>% 
  infer::rep_sample_n(size = 50, reps = 1000, replace = TRUE)
```
...
df_weight_avrg <- banana_samples %>% 
  ...
  dp$summarize(weight_avrg = mean(weight))
df_weight_avrg %>% 
  dp$slice_head(n = 20) %>% 
  knitr::kable(
               caption = "Average weight of 50 bananas in the first 5 samples 
               out of 1,000"
               ) %>% 
...
With each sample, Cy was able to plot the weight distribution 
and calculate the sample average. 
Figure \@ref(fig:dist-thousand) visualizes the first 20 of those 1,000 samples. 
Table \@ref(tab:average-thousand) lists the corresponding sample average 
for those 20 sample. 
Eyeballing these distributions and the sample averages, 
Cy learned the following:]{color="rosybrown"}


[1. The distributions look similar to each other, 
   and yet are distinguishable from one another, 
   confirming that 1,000 unique samples were taken from a population.]{color="rosybrown"}

[2. The sample averages calculated for each sample are close to each other, 
...
```{r dist-thousand-sample, echo=F, fig.cap="Distribution of 1,000 sample averages"}
hist_thousandsample <- gg$ggplot(df_weight_avrg, 
                                 gg$aes(x = weight_avrg)) + 
gg$geom_histogram(gg$aes(y = ..density..), boundary = 175, binwidth=1, 
                  colour="black", fill="white") + 
...
<!-- Plot a histogram with 1000 individual bananas for a demostration.  -->
...
title = 'Banana weights in ONE sample (A) \n and average weights from ONE-THOUSAND samples (B)'
  )
# maybe just use patchwork and scale_x_continuous, 
# combine hist_onesample and hist_thousandsample
```
...
And yet, when the 1,000 sample averages were plotted, 
...
sd_thousandsample <- df_weight_avrg %>% 
  dp$summarize(sd = sd(weight_avrg)) %>% 
  dp$pull(sd)

m_thousandsample <- df_weight_avrg %>% 
  dp$summarize(mean = mean(weight_avrg)) %>% 
  dp$pull(mean)
```


[After thoroughly examining the average weight from 1,000 samples, 
Cy estimates that a typical banana weighs `r m_thousandsample %>% round(0)` grams, 
and that this estimate has a margin of error 
of `r round(sd_thousandsample, 2)` grams.]{color="rosybrown"}
...
> 
  $$
  \sigma_\overline{x} = \frac{sd}{\sqrt{n}}
  (\#eq:sd)
  $$
  where $sd$ is the sample standard deviation

The Greek leter $\sigma$ is pronounced *sigma*. 
$\sigma_\overline{x}$ is pronounced *sigma eks bar*, 
and represents the standard deviation of $\overline{x}$. 
...
and I am 70% confident that the average weight of bananas 
is within `r round(m_thousandsample, 0)` 
$\pm$ `r round(sd_thousandsample, 2)`*.]{color="rosybrown"}
...
sounds much more undercertain than to say it is between 0 to 5&deg;C.  
...
from one sample to another much less compared to 
...
and a distribution of average weights from ONE-THOUSAND samples. 
...
Notice the scales of the x-axis are different in A and B. 
...
in Panel A. 
$\sigma_\overline{x}$ is represented by the distance between a dashed line and the solid line 
in Panel B. 
Equation \@ref(eq:sd) prescribes that 
$\sigma_\overline{x}$ is equal to $sd$ divided by $\sqrt{n}$. 
Substituting n with 50 from the banana study, 
$\sqrt{n}$ \approx 7$. 
so $\sigma_\overline{x}$ is about one seventh of $sd$, 
matching our visual observation ($1.5 \times 7 \approx 10$). 


Panel A of Figure \@ref(fig:sd-vs-se) is called a **sample distribution** 
whereas Panel B is called a **sampling distribution**. 
The word *sampling* is used to emphasize how a sampling distributioon typically arises: 
...
a standard error of the mean, $\sigma_\overline{x}$, 
is always a fraction of the sample standard deviation, $sd$. 
Note that standard error of mean is often shortened as Standard Error, or SE. 
...
### More terminology and notations

Here is a list of concepts instrumental to understanding the xxx. 
...
The uncertainty of this estimate is quantified by 
the standard error of mean, `r round(se, 2)` grams, 
derived from dividing the sample standard deviation by square root of sample size. 
